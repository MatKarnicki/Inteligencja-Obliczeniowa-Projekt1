...
def load_photos_as_arrays():
    ...
    for filename in os.listdir(load_folder):
        if filename.endswith('.png'):
            img = cv2.imread(os.path.join(load_folder, filename), cv2.IMREAD_COLOR)
...
saveFolder = (f"generated_images_color,hd={hidden_dimension} bs={batch_size},lr={lr},noise={noise_dimension}")
...
def save_generated_images(images, epoch):
    for idx, img in enumerate(images):
        ...
        image_unflat = img.detach().cpu().view(-1, 3, 128, 128)
...
class Discriminator(nn.Module):
    def __init__(self, image_dimension= 128 * 128 * 3,
                 hidden_dimension=hidden_dimension):
