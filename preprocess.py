...
def load_photos_as_tensors(directory_path):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.jpeg') or filename.endswith('.jpg') or filename.endswith('.png'):
            ...
            save_path = os.path.join(preprocessed_dir, f'sori_processed_{i}.png')
            torchvision.utils.save_image(img_transformed, save_path)
            i += 1
    return photo_tensors

photo_tensors = load_photos_as_tensors("preprocessed/")
